...
class FrozenLakeEnv(discrete.DiscreteEnv):
    ...
    def __init__(self, desc=None, map_name="4x4", rewarding=True, step_reward=-0.001, hole_reward=-10, is_slippery=True):
        if desc is None and map_name is None:
            ...
            for col in range(ncol):
                ...
                for a in range(4):
                    ...
                    if letter in b'GH':
                        ...
                        if is_slippery:
                            for act_num, b in enumerate([(a - 1) % 4, a, (a + 1) % 4]):
                                newrow, newcol = inc(row, col, b)
                                newstate = to_s(newrow, newcol)
                                newletter = desc[newrow, newcol]
                                done = bytes(newletter) in b'GH'
                                rew = float(newletter == b'G')*10.0
                                if self.rewarding:
                                    if newletter == b'F':
                                        rew = self.step_reward
                                    elif newletter == b'H':
                                        rew = self.hole_reward
                                if act_num == 0 or act_num == 2:
                                    li.append((1.0 / 10.0, newstate, rew, done))
                                else:
                                    li.append((8.0 / 10.0, newstate, rew, done))
                        else:
